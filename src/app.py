...
# ✅ 작업 상태 저장 딕셔너리 (Cloud Run은 stateless이므로, 실제 서비스에서는 DB 사용 권장)
job_status = {}
...
# ✅ Health Check 엔드포인트 (Cloud Run 404 에러 해결!)
...
def main_endpoint():
    ...
    if action == 'create_and_upload_shorts':
        ...
        }), 202 # 202 Accepted: 요청을 받았고 처리 중임을 알림
...
def generate_script_with_ai(topic="오늘의 명언", model_name="gemini-pro"):
    ...
    try:
        ...
        prompt = f"{topic}에 대한 짧고 매력적인 15초 유튜브 쇼츠 스크립트를 한국어로 작성해줘. 텍스트만."
...
def create_simple_video(script_text, output_path="output/shorts.mp4"):
    ...
    try:
        ...
        (
            ffmpeg
            .input('color=c=black:s=1080x1920', f='lavfi', t=15) # 15초 검은 배경 영상 (세로형)
            ...
            .run(overwrite_output=True, quiet=True) # quiet=True로 FFmpeg 로그를 줄임
        )
...
def upload_to_youtube(video_path, title, description, tags):
    ...
    if not video_path or not os.path.exists(video_path):
        logger.error("업로드할 영상 파일이 유효하지 않습니다.")
        return False
    ...
    try:
        ...
        logger.info(f"YouTube 업로드 성공! 영상 ID: {response.get('id')}")
        return True
    except Exception as e:
...
def process_youtube_shorts_upload(metadata, job_id):
    ...
    try:
        ...
        video_title = f"✨오늘의 긍정 에너지: {script[:30]}..." # 스크립트 앞부분 사용
        video_description = (
            f"매일 긍정적인 메시지로 하루를 시작하세요! #긍정명언 #동기부여 #쇼츠\n\n"
            f"당신의 하루를 바꿀 수 있는 제품 보러가기: {AFFILIATE_LINK}"
        )
        video_tags = ["명언", "동기부여", "긍정", "쇼츠", "매일명언", script_topic.replace(" ", "")]

        # 3. 영상 생성
        output_dir = "output"
        os.makedirs(output_dir, exist_ok=True) # output 폴더가 없으면 생성
        video_file = os.path.join(output_dir, f"daily_shorts_{job_id}.mp4")
        created_video_path = create_simple_video(script, video_file)
        ...
        if os.path.exists(created_video_path):
            ...
            logger.info(f"임시 영상 파일 삭제 완료: {created_video_path}")
        ...
        # 혹시 모를 임시 파일 정리 (에러 발생 시에도)
        if 'created_video_path' in locals() and os.path.exists(created_video_path):
            os.remove(created_video_path)
            logger.info(f"Finally 블록에서 임시 영상 파일 삭제 완료: {created_video_path}")
...
if __name__ == "__main__":
    ...
    # Cloud Shell에서 테스트 시 'http://0.0.0.0:8080' 대신
    # 'http://127.0.0.1:8080' 또는 Cloud Shell의 '웹 미리보기' 기능을 사용하세요.
