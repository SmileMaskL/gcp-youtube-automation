...
def trigger_youtube_upload(request):
    ...
    niche_keywords = ["신기한 과학 사실", "역사 속 숨겨진 이야기", 
                     "최신 기술 트렌드", "일상 속 꿀팁", "건강 상식"]
    ...
    try:
        created_video_path = os.path.join(temp_dir, "temp_created_video.mp4")
        create_short_video(background_video_path, audio_file_path, created_video_path)
        logger.info(f"기본 영상 생성 완료: {created_video_path}")

        edit_video_for_shorts(created_video_path, content_topic, suggested_title)
        os.rename(created_video_path, final_video_file_path)
    ...
    video_tags = [tag.strip() for tag in 
                 f"shorts,AI,자동화,유튜브,꿀팁,수익화,정보,{selected_niche},{suggested_title.replace(' ', '')}".split(',') 
                 if tag.strip()]
