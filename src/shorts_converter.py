...
class ShortsConverter:
    ...
    def detect_interesting_segments(
            self, video_path: str, info: Dict) -> List[Tuple[float, float]]:
        """흥미로운 구간 자동 감지"""
        segments = []
    ...
    def _detect_audio_peaks(self, video_path: str,
    ...
    def convert_segment_to_shorts(self, video_path: str, start_time: float,
        ...
        try:
            input_stream = ffmpeg.input(
                video_path, ss=start_time, t=end_time - start_time)

            # 세로형으로 크롭 및 리사이즈
            video = (
                input_stream .video .filter(
                    'scale',
                    self.shorts_width,
                    self.shorts_height,
    ...
    def create_shorts_from_video(
        ...
        logger.info(
            f"비디오 정보 - 길이: {info['duration']:.1f}초, 해상도: {info['width']}x{info['height']}")
