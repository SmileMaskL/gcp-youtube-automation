...
def generate_content_with_openai(config_instance, prompt_text, model="gpt-4o"):
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a helpful assistant that "
                                              "generates concise and engaging content for "
                                              "YouTube Shorts. Provide a title and content "
                                              "in JSON format."},
        ...
        return {"title": "AI 콘텐츠 생성 오류", "content": "콘텐츠를 생성하는 데 "
...
def generate_content_with_gemini(gemini_api_key, prompt_text, model="gemini-1.5-flash"):
    ...
    try:
        ...
        full_prompt = (
            f"{prompt_text}\n\n"
            f"Your response MUST be a JSON object with two keys: 'title' (string) "
            f"and 'content' (string). Example: {{\"title\": \"추천 제목\", "
            f"\"content\": \"생성된 내용\"}}"
        )
...
def generate_niche_content(config_instance, niche_keyword, model_preference="openai"):
    ...
    prompt = (
        ...
        f"유용한 팁, 또는 짧은 스토리를 1개 작성해주세요. 제목과 내용을 포함하는 "
        f"JSON 형식으로 응답해주세요. 내용은 50단어 이내로 간결하게 작성해주세요. "
        f"예시: {{\"title\": \"{niche_keyword}의 놀라운 비밀\", \"content\": "
        f"\"{niche_keyword}에 대한 놀라운 사실입니다...\"}}"
    )
