...
class AIRotationManager:
    ...
    def _init_clients(self):
        if GEMINI_API_KEY:
            ...
            log_system_health(
                "Gemini client initialized.", level="info"
            )
        else:
            log_system_health(
                "Gemini API Key not found. Gemini client will not be available.",
                level="warning"
            )
        ...
        try:
            ...
            log_system_health(
                "OpenAI client initialized with first key.", level="info"
            )
        except Exception as e:
            log_system_health(
                f"Error initializing OpenAI client: {e}. "
                "OpenAI client will not be available.",
                level="error"
            )
            self.openai_client = None

    def generate_content(
        self, prompt, model_preference=None, max_tokens=1000, temperature=0.7
    ):
        ...
        for _ in range(2):
            if selected_model == "gpt-4o":
                if api_usage_tracker.check_limit(
                    "openai", api_usage_tracker.get_usage("openai"),
                ...
                if api_usage_tracker.check_limit(
                    "gemini", api_usage_tracker.get_usage("gemini"),
                    ...
                    if self.gemini_client:
                        try:
                            ...
                            log_system_health(
                                "Gemini로 콘텐츠를 성공적으로 생성했습니다.",
                                level="info"
                            )
                            return response.text
                        except Exception as e:
                            log_system_health(
                                f"Gemini 콘텐츠 생성 오류: {e}. "
                                "다른 모델로 전환합니다.",
                                level="error"
                            )
                    ...
                    log_system_health(
                        "Gemini 일일 사용 한도 초과. GPT-4o로 전환합니다.",
                        level="warning"
                    )
                    selected_model = "gpt-4o"
            ...
            if model_preference:
                ...
                log_system_health(
                    f"다음 모델인 '{selected_model}'로 다시 시도합니다.",
                    level="info"
                )
