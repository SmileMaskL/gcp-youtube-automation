...
GCP_PROJECT_ID="youtube-fully-automated" # <-- 여기에 실제 프로젝트 ID 입력!
...
add_iam_binding() {
  ...
  gcloud projects add-iam-policy-binding "${GCP_PROJECT_ID}" \
    ...
    --role="${role}" \
    --condition=None \
    --quiet >/dev/null && echo "✅ ${role} 권한 추가 완료 또는 이미 존재함." || echo "❌ ${role} 권한 추가 실패!"
}
...
echo "============================================================"
echo "👉 최종 확인 명령어: 이 명령어를 꼭 실행해서 권한이 제대로 부여되었는지 확인하세요!"
